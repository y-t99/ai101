...
为了理解ICL，近期的研究致力于探究影响ICL性能的因素（Shin et al., 2022; Yoo et al., 2022; Kossen et al., 2023）以及ICL的工作原理（Dai et al., 2023a; Irie et al., 2022）。在本章节中，我们将详细阐述影响ICL的各种因素和其学习机制，如图4所示。

![image-20250810132029279](./picture/icl-factor.png)图四，与 ICL 性能相关性较强的因素总结，以及从不同角度解释 ICL 为何有效的不同观点。

#### 影响因素 (Influencing Factors)

我们在此讨论了在预训练阶段和推理阶段影响ICL性能的相关研究。

##### 预训练阶段

我们首先介绍影响预训练阶段的因素。预训练语料的**多样性**显著影响ICL的性能（Shin et al., 2022; Yadlowsky et al., 2023; Raventós et al., 2023）。特别地，Shin等人（2022）发现，源领域比语料库的大小更重要，这表明结合多种语料可能有助于ICL能力的涌现。类似地，Raventós等人（2023）通过实验发现了一个任务多样性的阈值，当超过这个阈值后，LLM在未见过的任务上会展现出强大的ICL能力。另一项研究则探讨了**数据分布**对ICL的影响（Chan et al., 2022; Wies et al., 2023）。例如，Chan等人（2022）证明，当训练数据呈现出特定的分布特性时，ICL能力便会涌现，例如“爆发性”（burstiness），即项目以簇的形式出现，而非均匀分布。

除了这些工作，一些研究还探讨了**模型架构**和**训练过程**对ICL性能的影响（Wei et al., 2022b; Brown et al., 2020; Ding et al., 2024）。Wei等人（2022b）研究了许多大规模模型在多任务上的涌现能力。他们认为，当预训练模型的训练步数或参数量达到一定规模时，它就会获得一些涌现的ICL能力。Ding等人（2024）指出，在推理过程中，上下文中的样本应该相互关注，这表明当前的因果语言模型（Causal LLMs）可能会导致次优的ICL性能。

##### 推理阶段

在推理过程中，示例样本的多种属性也会影响ICL的性能。Min等人（2022c）证明，输入-标签的设置，例如配对格式、标签空间的覆盖范围以及输入分布，都对ICL性能有显著贡献。然而，与Min等人（2022c）认为输入-标签映射关系对ICL影响不大的结论相反，后来的研究表明，**准确的映射关系**会显著影响ICL的性能（Yoo et al., 2022; Pan et al., 2023a; Tang et al., 2023a）。Wei等人（2023b）进一步指出，即使是翻转的或语义不相关的输入-标签映射，模型也能够学会。

从示例构建的角度来看，近期的文献关注于示例的**多样性**和**简洁性**（An et al., 2023），样本的**顺序**（Lu et al., 2022; Zhang et al., 2022b; Liu et al., 2023b），以及示例与查询之间的**相似性**（Liu et al., 2022）。例如，Liu等人（2022）发现，嵌入向量与查询样本更接近的示例通常比那些距离较远的示例能带来更好的性能。值得注意的是，尽管研究人员努力优化示例以提升性能，ICL推理过程中仍然存在明显的**特征偏差**（Si et al., 2023）。如何克服强烈的先验偏差，并确保模型对所有上下文信息给予同等权重，仍然是一个挑战（Kossen et al., 2023）。

#### Learning Mechanism

从学习机制的视角，我们深入探讨了阐释ICL为何有效的研究。

##### Functional Modules

ICL能力与Transformer内部的特定功能模块紧密相关。作为核心组件之一，**注意力模块**是研究ICL机制的焦点（Olsson et al., 2022; Bietti et al., 2023; Dai et al., 2023a; Irie et al., 2022; Li et al., 2023c; Gao et al., 2023; Zhang et al., 2023b）。特别地，Olsson等人（2022）识别出了一些特定的注意力头，称之为**“归纳头”（induction heads）**，它们可以复制先前的模式以预测下一个词元，从而逐步发展出ICL能力。此外，Wang等人（2023b）关注了Transformer中的信息流，发现在ICL过程中，示例中的**标签词**充当了“锚点”（anchors），负责聚合和分发关键信息，用于最终的预测。

##### Theoretical Interpretation

在本小节，我们介绍从不同角度对ICL的理论解释。

**贝叶斯视角 (Bayesian View)** 在贝叶斯框架下，ICL被解释为一种**隐式的贝叶斯推理**，模型通过识别示例间的共享潜在概念来执行ICL（Xie et al., 2022; Wies et al., 2023; Ahuja et al., 2023; Jiang, 2023; Wang et al., 2023e）。其他观点认为，LLM通过其注意力机制编码了贝叶斯模型平均（Bayesian Model Averaging）算法（Zhang et al., 2023b）。随着上下文示例数量的增加，隐式贝叶斯推理变得类似于核回归（kernel regression）（Han et al., 2023a）。

**梯度下降视角 (Gradient Descent View)** 梯度下降为理解ICL提供了另一个有价值的视角。Dai等人（2023a）发现了Transformer的注意力和梯度下降之间的一种对偶形式，并指出基于GPT的ICL其行为在多个方面与显式微调相似。其他研究试图在简化的回归设定中建立ICL与梯度下降之间的联系（von Oswald et al., 2023; Ahn et al., 2023; Mahankali et al., 2023; Li et al., 2023c）。例如，von Oswald等人（2023）表明，手动构建参数的、仅包含线性注意力的Transformer与通过梯度下降学习到的模型密切相关。Li等人（2023c）发现，仅包含自注意力的Transformer与通过梯度下降训练的模型具有相似性。然而，这些研究中使用的简化设定引发了关于这些发现在真实世界场景中是否直接适用的争论（Shen et al., 2024）。Fu等人（2023）则认为，Transformer在处理线性回归时，是通过更高阶的优化技术而非梯度下降来执行ICL的。

**其他视角 (Other Views)** 除了将ICL与单一算法联系起来，研究人员还从多个角度对其进行分析，包括**能力解耦**、**算法学习**和**信息论**。Pan等人（2023b）将ICL能力解耦为**任务识别能力**和**任务学习能力**，每种能力在不同条件下表现不同。另一种典型理论将ICL抽象为一个算法学习问题（Akyürek et al., 2023; Garg et al., 2022; Li et al., 2023e; Bai et al., 2023b），其中Transformer会动态地为不同的ICL实例选择合适的算法，例如梯度下降或岭回归。此外，Hahn和Goyal（2023）利用信息论，在语言学动机的假设下，为ICL给出了一个误差界限，解释了下一个词元预测如何带来ICL能力。

​	这些分析研究为解释 ICL 迈出了重要的一步。然而，它们大多侧重于简单任务和小型模型。将分析扩展到广泛的任务和大型模型可能是下一步值得考虑的方向。
